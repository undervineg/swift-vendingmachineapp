...
이럴 때 각자 객체 입장에서 생각해볼 필요가 있습니다.

>- DataSource 입장: 언제 데이터가 초기화되고, 데이터가 갱신되나, 그래서 언제 다시 그리라고 요청해야 하나, 전체를 다 갱신해야 하나 일부만 갱신해야 하나...
>- View 입장: 언제 DataSource가 설정되고, 언제 데이터를 확인해서 그릴까, 전체를 갱신하는 시점과 일부만 갱신하는 시점 동작 구분이 필요한가...
>- 아직 해답을 모른다고 해도 지금도 나쁘지 않습니다. 테이블뷰에 대해 공부해보면 조금 힌트가 생길겁니다.
